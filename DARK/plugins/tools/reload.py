...
from DARK.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await KUZE.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await KUZE.stop_stream_force(chat_id)
        except:
            pass
             ...
             [
                 [
                      InlineKeyboardButton(
                         "• 𝖧ᴀᴄᴋᴇᴅ вʏ  •", url=f"https://t.me/ll_KUZE_ll")
                 ]
            ]
         ),
     )
